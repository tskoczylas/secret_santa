...
export default function SecondStep() {
    ...
    const defaultSwitchStatus = () => {
        if (sendNowStorage === null) {
            ...
            return JSON.parse(sendNowStorage).data
        }

    }
    const defaultStartDate = () => {
        if (sendNowStorage !== null) {
            return new Date(startDateStorage)
        } else {
            return startDate
        }
    ...
    const defaultLastResponseDate = () => {
        if (sendNowStorage !== null) {
            return new Date(lastResponseDateStorage)
        } else {
        ...
        <Grid container
            ...
            <Grid item>
                <Box sx={{display: 'flex', alignItems: 'flex-end', marginInline: 2}}>
                    ...
                    <HelpOutlineIcon onClick={() => {
                        alert(textTemplate.newGame.secondStep.send_invitation_info)
                    }} sx={{ml: 2, my: 0.5}} fontSize="small">add_circle</HelpOutlineIcon>
                </Box>
            ...
            <Grid item>
                <Box sx={{display: 'flex', alignItems: 'flex-end', marginInline: 2}}>
                    ...
                    <HelpOutlineIcon onClick={() => {
                        alert(textTemplate.newGame.secondStep.last_response_info)
                    }} sx={{ml: 2, my: 0.5}} fontSize="small">add_circle</HelpOutlineIcon>
                </Box>
